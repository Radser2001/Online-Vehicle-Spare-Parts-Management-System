...
public class ItemUpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * call the updateItemDetails details method in ItemDBUtil and get the boolean
		 * value returned
		 */
		boolean itemInserted = ItemDBUtil.updateItemDetails(sellerID, itemID, title, status, description, category,
				model, quantity, price, mainImage, subImage1, subImage2, subImage3);

		// if the item is updated
		if (itemInserted) {

			dispatcher = request.getRequestDispatcher("sellerDashboard.jsp");

			request.setAttribute("itemUpdateStatus", "success");

		} else {
			// if item updating is failed
			request.setAttribute("itemUpdateStatus", "failed");
		}
