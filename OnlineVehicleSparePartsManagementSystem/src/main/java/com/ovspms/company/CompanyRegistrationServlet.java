...
public class CompanyRegistrationServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * call the insertCompany details method in companyDBUtil and get the boolean
		 * value returned
		 */ boolean companyInserted = companyDBUtil.insertCompanyDetails(sellerID, name, contactNumber, email, logo,
				address, district, city);

		// if the company is inserted
		if (companyInserted) {
			Company company = companyDBUtil.getCompanyInfo(sellerID);
			dispatcher = request.getRequestDispatcher("companyRegistration.jsp");
			request.setAttribute("company", company);
			request.setAttribute("status", "success");

		} else {
			// if company inserting is failed
